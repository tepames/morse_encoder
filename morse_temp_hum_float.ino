...
morse_xe1bep xe1bep(9,1500,40); //object for coding morse funcions first is pin, second is frequency and third value of dot in milliseconds
...
void loop()
{
  ...
  if (ledState == LOW) {
    if(currentMillis - previousMillis > intervalOff) {
            ...
            xe1bep.numToMorseFloat(humedad);
            xe1bep.lSpace();
            //separating the values with /
            xe1bep.diagonal();
            xe1bep.lSpace();
            //parsing temperature to morse
            xe1bep.numToMorseFloat(temperatura);
            xe1bep.lSpace();
            //turn off the led
            digitalWrite(13, LOW);
    }
